...
/*   Updated: 2021/12/07 16:42:36 by jmaia            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	conv_d(t_dynamic_buffer *output_line_buffer, t_options *options, int param)
{
	...
	err = conv_d_returns(output_line_buffer,
			options, left_char, param, &raw_data);
	free(raw_data);
	return (err);
}

int	conv_d_returns(t_dynamic_buffer *output_line_buffer,
		t_options *options, char left_char, int param, unsigned char **raw_data)
{
	int	err;
