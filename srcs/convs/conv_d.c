...
/*   Updated: 2021/12/03 18:03:55 by jmaia            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	const unsigned char	*raw_nbr;
	const unsigned char	*raw_data;
	char				left_char;

	raw_nbr = ft_itoa(param);
	if (!raw_data)
		return (1);
	left_char = (t_options->flags & FLAG_ALWAYS_SIGN) * '+';
	left_char += !left_char * (t_options->flags & FLAG_BLANK);
	if (left_char)
	{
		raw_data = append_left_char(left_char, raw_nbr);
		free(raw_nbr);
		if (!raw_data)
			return (1);
	}
